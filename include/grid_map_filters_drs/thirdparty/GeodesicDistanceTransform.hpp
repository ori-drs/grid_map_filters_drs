...
template <class T>
void GeodesicDistance(const cv::Mat& image, const std::vector<cv::Point>& seeds, cv::Mat& output, int max_visits = -1) {
  ...
  auto __compare = [](const __queue_cell<T>& a, const __queue_cell<T>& b) { return a.dist > b.dist; };
  std::priority_queue<__queue_cell<T>, std::vector<__queue_cell<T>>, decltype(__compare)> que(__compare);
  ...
  constexpr unsigned char __CELL_VISITED = (unsigned char)(255), __CELL_NOT_VISITED = (unsigned char)(0);
  auto __update_cell = [area, &image, &u, image_data, &que, dist, &cell_status, &__eikonal_update, __CELL_VISITED, __CELL_NOT_VISITED]() {
